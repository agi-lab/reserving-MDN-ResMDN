############################
# 0.2 FUNCTIONS_PROCESS ####
############################
# This module:
# - Designs functions used for other non-plotting purposes, such as triangle partitions, quantile estimation, etc.
...
Normalise = function(Full_Data){
  ...
  ###CATEGORICAL MAPPING FOR RESMDN
...
Simulate_MG <- function(table, start,   trials, AQ_chosen, DQ_chosen, Simulations){
  sim_vec = c()
  
  table1 =  as.data.frame(table[AQ == AQ_chosen & DQ == DQ_chosen,])
  for (i in 1:Simulations){
    ...
    #print(i)
    UNIFORM = runif(1)
    cumul_alpha = table1[index]/trials
    while (UNIFORM > cumul_alpha){
      index = index + 3
      cumul_alpha = cumul_alpha + table1[index]/trials
    }
    ...
    if (Distribution == "Log"){
      ...
      sim_vec[i] = (rnorm(1,as.numeric(table1[index+1]), as.numeric(table1[index+2])))
